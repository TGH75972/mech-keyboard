document.addEventListener('DOMContentLoaded', () => {
    ...
    keys.forEach(key => {
        ...
        key.addEventListener('mouseup', () => {
            handleVirtualKeyRelease(key.dataset.key);
        });
    ...
    altKey.addEventListener('mousedown', () => {
        if (!altActive) {
            altActive = true;
            playKeySound('default');
            toggleMusic();
    ...
    if (clearKey) {
        clearKey.addEventListener('mousedown', () => {
            playKeySound('keypress');
            clearInputField();
        });

    } else {
        console.error('Clear key element not found.');
    }
